...
export async function getAllCases(preview) {
  const results = await getClient(preview)
    .fetch(`*[_type == "case"  && published==true] {
      ${caseFields}
    }`);
  return results;
}
...
export async function getAllOverOns(preview) {
  const results = await getClient(preview)
    .fetch(`*[_type == "overOns"] | order(order asc){
      ${overonsFields}
    }`);
  return results;
}
...
export async function getAllSettings(preview) {
  const results = await getClient(preview).fetch(`*[_type == "settings"] {
      ${settingsFields}
    }`);
  return results;
}
